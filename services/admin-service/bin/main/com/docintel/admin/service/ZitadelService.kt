...
class ZitadelService(
    ...
    fun updateUserRole(userId: String, tenantId: String, role: String) {
        ...
        try {
            val existing = findUserGrant(userId, orgId)
            if (existing != null) {
                client.put().uri("/management/v1/users/$userId/grants/${existing.id}")
                    ...
                    .timeout(timeout)
                    .block()
                log.info("Updated role for user $userId in tenant $tenantId to $role (grantId=${existing.id})")
            } else {
                client.post().uri("/management/v1/users/$userId/grants")
                    ...
                    .timeout(timeout)
                    .block()
                log.info("Created grant for user $userId in tenant $tenantId with role $role")
            }
        } catch (e: Exception) {
    ...
    private fun getUserRole(userId: String, orgId: String): String {
        val grant = findUserGrant(userId, orgId) ?: return "tenant_user"
        return when {
            "platform_admin" in grant.roleKeys -> "platform_admin"
            "tenant_admin" in grant.roleKeys -> "tenant_admin"
            else -> "tenant_user"
        }
    }

    /**
     * Fetch all user grants, then return the first grant matching our project.
     * We do NOT use URL query params to filter (Zitadel v1 Management API ignores them
     * for GET endpoints; the correct search is via POST _search or client-side filtering).
     */
    private fun findUserGrant(userId: String, orgId: String): ZitadelGrant? {
        return try {
            val resp = client.get()
                .uri("/management/v1/users/$userId/grants")
                .header("x-zitadel-orgid", orgId)
                .retrieve()
                .bodyToMono<ZitadelGrantListResponse>()
                .timeout(timeout)
                .block()
            resp?.result?.firstOrNull { it.projectId == projectId }
        } catch (e: Exception) {
            log.warn("Failed to fetch grants for user $userId in org $orgId: ${e.message}")
            null
        }
    }

...
class TenantManagementService(
    ...
    fun createTenant(req: CreateTenantRequest): TenantSummary {
        jdbcTemplate.update(
            """
            INSERT INTO tenants (id, name, quota_documents, quota_queries_per_day, settings)
            VALUES (?, ?, ?, ?, jsonb_set('{}', '{llm_model}', to_jsonb(?::text)))
            """.trimIndent(),
    ...
    fun updateUserRole(tenantId: String, userId: String, req: UpdateUserRoleRequest): Boolean {
        ...
        return try {
            ...
            log.error("Role update failed for user $userId in tenant $tenantId: ${e.message}")
            false
        }
    }

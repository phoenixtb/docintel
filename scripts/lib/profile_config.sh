...
# write_profile_override PROFILE
# ------------------------------------------------------------------------------
# Upsert a single KEY=VALUE line in .docintel-profile, preserving other keys.
# This lets PROFILE (GPU profile) and PLATFORM (target arch) be persisted
# independently in the same file without clobbering each other.
_profile_file_set() {
    local _key="$1" _val="$2" _tmp
    _tmp="$(mktemp)"
    if [ -f "$DOCINTEL_PROFILE_FILE" ]; then
        grep -v "^${_key}=" "$DOCINTEL_PROFILE_FILE" 2>/dev/null > "$_tmp" || true
    else
        printf '# Auto-generated by DocIntel. Delete this file to restore auto-detection.\n' > "$_tmp"
    fi
    printf '%s=%s\n' "$_key" "$_val" >> "$_tmp"
    mv "$_tmp" "$DOCINTEL_PROFILE_FILE"
}

write_profile_override() {
    _profile_file_set PROFILE "$1"
    _profile_file_set SET_AT "$(date -u +"%Y-%m-%dT%H:%M:%SZ" 2>/dev/null || date -u)"
}

write_platform_override() {
    _profile_file_set PLATFORM "$1"
    _profile_file_set SET_AT "$(date -u +"%Y-%m-%dT%H:%M:%SZ" 2>/dev/null || date -u)"
}
...
torch_vars_for_profile() {
    ...
    # Pin the PyTorch local version label (+cpu / +cu128 / …) so the wheel can
    # ONLY come from the matching index. Without it, `torch==2.11.0` also exists
    # on PyPI as the CUDA build, and uv pulls that (dragging in ~5 GB of
    # nvidia-* wheels) even for the cpu profile.
    export TORCH_VERSION="${TORCH_VERSION_DEFAULT}+${_p}"
    export PROFILE_TAG="${_p}"
}

# ------------------------------------------------------------------------------
# _native_docker_platform
# Maps the host architecture (uname -m) to a Docker platform string.
# ------------------------------------------------------------------------------
_native_docker_platform() {
    case "$(uname -m)" in
...
# resolve_platform
#
# Single source of truth for the Docker target platform, resolved with the same
# precedence model as the GPU profile so build.sh, start.sh and any other caller
# consume ONE decision instead of each hard-coding a value.
#
# Precedence:
#   1. DOCKER_DEFAULT_PLATFORM env (explicit, e.g. set in shell or .env)
#      → persisted to .docintel-profile so later runs remember it.
#   2. PLATFORM= line in .docintel-profile (a previously persisted choice).
#   3. host-native (default) → exported EMPTY so Docker/compose use the host
#      arch with no emulation. Native is intentionally not persisted (mirrors
#      how auto-detected profiles aren't persisted).
#
# Exports: DOCKER_DEFAULT_PLATFORM (value or "" = native)
# Sets:    DOCKER_PLATFORM_SOURCE (env|file|native), DOCKER_PLATFORM_LABEL
# ------------------------------------------------------------------------------
resolve_platform() {
    DOCKER_PLATFORM_SOURCE="native"
    local _plat=""

    if [ -n "${DOCKER_DEFAULT_PLATFORM:-}" ]; then
        _plat="$DOCKER_DEFAULT_PLATFORM"
        DOCKER_PLATFORM_SOURCE="env"
        write_platform_override "$_plat"
    elif [ -f "$DOCINTEL_PROFILE_FILE" ]; then
        local _fp
        _fp=$(grep "^PLATFORM=" "$DOCINTEL_PROFILE_FILE" 2>/dev/null | cut -d= -f2 | tr -d '[:space:]')
        if [ -n "$_fp" ]; then
            _plat="$_fp"
            DOCKER_PLATFORM_SOURCE="file"
        fi
    fi

    # Native (empty) lets Docker pick the host arch — fastest, no emulation.
    export DOCKER_DEFAULT_PLATFORM="$_plat"
    if [ -n "$_plat" ]; then
        DOCKER_PLATFORM_LABEL="$_plat"
    else
        DOCKER_PLATFORM_LABEL="$(_native_docker_platform)"
    fi
    export DOCKER_PLATFORM_SOURCE DOCKER_PLATFORM_LABEL
}
...
#   4. docker-compose.storage.yml   (only when DOCINTEL_DATA_DIR is set — puts ALL
#                                    DocIntel persistent data under that directory)
#
...
compose_file_chain() {
    ...
    # Optional: relocate ALL DocIntel data under DOCINTEL_DATA_DIR. Bind mounts
    # require the target dirs to pre-exist, so create every one declared in the
    # overlay before any `up`. The overlay's `device:` lines are the single
    # source of truth for the subdirectory list.
    if [ -n "${DOCINTEL_DATA_DIR:-}" ] && [ -f "${_root}/docker-compose.storage.yml" ]; then
        local _sub
        while IFS= read -r _sub; do
            [ -n "$_sub" ] && mkdir -p "${DOCINTEL_DATA_DIR}/${_sub}" 2>/dev/null || true
        done < <(grep -oE 'device: \$\{DOCINTEL_DATA_DIR\}/[A-Za-z0-9_-]+' "${_root}/docker-compose.storage.yml" | sed 's#.*/##')
        COMPOSE_FILES="${COMPOSE_FILES} -f ${_root}/docker-compose.storage.yml"
    fi
